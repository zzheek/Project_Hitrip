...
document.addEventListener("DOMContentLoaded", function() {
	  ...
	  function generateCalendar(planyear, planmonth) {
		    ...
		    let planday = 1;
		    const startDate = new Date(planyear, planmonth, 1); // Create a Date object for the start date
		    const endDate = new Date(planyear, planmonth + 1, 0); // Create a Date object for the end date
		        ...
		        if ((i === 0 && j < startDate.getDay()) || planday > endDate.getDate()) {
		          ...
		          for (const item of planList) {
		            if (
		              planyear === item.planyear &&
		              planmonth === item.planmonth - 1 &&
		              planday >= item.planday && // Check if planday is within the range
		              planday <= item.planeday
		            ) {
		              if (titleCount < 2) { // Only show up to 3 titles
		                ...
		                titleCount++;
		            	  ...
		            	  if (titleCount === 2) {
		            		  ...
		            		  moreLink.href = "#"; // Replace with your actual URL
		            		  moreLink.style.color = "blue";
		            		  moreLink.style.fontSize = "12px";
		            		  moreLink.style.textDecoration = "none";

		            		  const br = document.createElement("br"); // Add a line break before the "more+" link
